    ...
    <form v-else class="space-y-4 mt-4 w-full">
      <div class="input-group">
        <div class="flex items-center justify-start gap-x-2">
          ...
          <p class="">Check this box if you want to make this chapter free for preview.</p>
        </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const isFree = ref(!!props.chapter?.is_free)
    const originalIsFree = ref(!!props.chapter?.is_free)
    ...
    const onSubmit = async () => {
      ...
      const res = await updateCourseChapter(props.slug, props.id, { is_free: isFree.value })
