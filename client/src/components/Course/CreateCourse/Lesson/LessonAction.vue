<template>
  <div class="flex items-center gap-3">
    <button
      ...
      @click.prevent="onChangeLesson()"
      class="px-4 h-10 min-h-10 text-sm text-whiteColor bg-blackColor rounded-md"
      :class="{ 'opacity-75': isSubmitting || !isComplete }"
    >
      {{ lesson?.is_published ? 'Unpublish' : 'Publish' }}
    </button>
    <label :for="'my_modal_' + lesson?.id" class="px-3 h-10 min-h-10 flex items-center justify-center text-whiteColor bg-blackColor rounded-md cursor-pointer">
      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none">
        ...
        /></svg
    ></label>
    <input type="checkbox" :id="'my_modal_' + lesson?.id" class="modal-toggle" />
    <div class="modal" role="dialog">
      ...
      <label class="modal-backdrop" :for="'my_modal_' + lesson?.id">Close</label>
    </div>
  </div>
</template>
...
import { publishLesson, unpublishLesson, deleteLesson } from '@/webServices/lessonService'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const onChangeLesson = async () => {
      isSubmitting.value = true

      if (!props.lesson?.is_published) {
        const res = await publishLesson(props.slug, props.chapterId, props.lessonId)
        if (!res.success) {
        ...
        if (res.success) {
          homeStore.onChangeToast({ show: true, type: 'success', message: 'Lesson published Successfully !' })
        }
      } else {
        const res = await unpublishLesson(props.slug, props.chapterId, props.lessonId)
        ...
        if (res.success) {
          homeStore.onChangeToast({ show: true, type: 'success', message: 'Lesson unpublished Successfully !' })
        }
      }
    ...
    const onDeleteLesson = async () => {
      ...
      const res = await deleteLesson(props.slug, props.chapterId, props.lessonId)
      ...
      isSubmitting.value = false
      homeStore.onChangeToast({ show: true, type: 'success', message: 'Lesson deleted Successfully !' })
      router.push({ name: 'create-course-chapter', params: { slug: props.slug, chapterId: props.chapterId } })
    }

    return { isSubmitting, onChangeLesson, onDeleteLesson }
  }
})
</script>
