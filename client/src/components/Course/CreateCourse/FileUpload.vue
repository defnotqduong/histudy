  ...
  <div class="mt-6 py-4 px-6 bg-lighterColor rounded-md">
    <div class="font-bold text-headingColor flex items-start justify-between">
      ...
      <button @click.prevent="toggleEdit" class="flex items-center gap-2">
        ...
        <template v-else-if="!isEditting && thumbUrl">
    ...
    <template v-if="!isEditting">
      <div v-if="!thumbUrl" class="mt-6 flex items-center justify-center h-72 bg-slate-200 rounded-md border-2 border-dashed border-[#575767]">
    ...
    <template v-else>
      ...
      <div v-if="errors?.thumbnail && errors?.thumbnail.length > 0">
        <p v-for="(err, index) in errors?.thumbnail" :key="index" class="mt-2 text-red-500">{{ err }}</p>
      </div>
