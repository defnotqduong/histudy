          ...
          <div class="grid grid-cols-12 mt-2 mb-6 gap-2">
            <div v-for="(permission, index) in permissions" :key="index" class="col-span-3 flex items-center">
              <div class="checkbox-wrapper">
                <input type="checkbox" :id="'cbx' + permission.id" @change="togglePermission(permission.id)" class="inp-cbx" />
...
export default defineComponent({
  ...
  setup() {
    ...
    const onSubmit = async () => {
      ...
      const res = await createRole({ name: name.value, permissions: chossePerms.value })
