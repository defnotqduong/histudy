<template>
  <div class="pt-10 pb-40">
    ...
    <div v-if="!loading">
      ...
      >
      <div class="mt-4 mb-8">
        <h5 class="text-lg text-headingColor font-bold">Edit User</h5>
          ...
          <div class="grid grid-cols-12 mt-2 mb-6 gap-2">
            <div v-for="(role, index) in roles" :key="index" class="col-span-3 flex items-center">
              <div class="checkbox-wrapper">
                <input type="checkbox" :id="'cbx' + role.id" @change="toggleRole(role.name)" :checked="isChecked(role.name)" class="inp-cbx" />
...
import { getUser, updateUser } from '@/webServices/userService'
...
export default defineComponent({
  ...
  setup() {
    ...
    const isChecked = role => {
      return chosseRoles.value.some(r => r === role)
    }
    ...
    const fetchData = async () => {
      ...
      const [userData, roleData] = await Promise.all([getUser(id.value), getAllRole()])
      ...
      if (userData.success) {
        ...
        chosseRoles.value = userData.user.roles
      }

      if (roleData.success) roles.value = roleData.roles
      loading.value = false
    }
