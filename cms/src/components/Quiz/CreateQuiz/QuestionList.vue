<template>
  <div class="mt-6">
    <draggable v-model="questions" ghost-class="ghost" @start="drag = true" @end="onDragEnd" @change="onChange" itemKey="id">
      <template #item="{ element: question }">
        <div class="bg-whiteColor px-3 py-3 border text-headingColor rounded-md mb-4 text-sm cursor-pointer">
          <div class="flex items-center justify-between gap-x-3">
            ...
            <div class="flex items-center gap-x-2">
              <button @click.prevent="" class="px-3 py-2 text-primaryColor bg-primaryOpacityColor rounded-md">
              ...
              <button class="px-3 py-2 text-dangerColor bg-dangerOpacityColor rounded-md cursor-pointer">
          ...
          <ul class="mt-2">
            <li v-for="ans in question.answers" :key="ans.id">
              <input type="radio" :checked="ans.is_correct" />
