  ...
  <div class="mt-4 py-4 px-6 bg-lighterColor rounded-md">
    <div class="font-bold text-headingColor flex items-start justify-between">
      ...
      <button @click.prevent="toggleEdit" class="flex items-center gap-2">
        ...
        <template v-else-if="!isEditting && logoUrl">
    ...
    <template v-if="!isEditting">
      <div v-if="!logoUrl" class="mt-6 flex items-center justify-center h-72 bg-slate-200 rounded-md border-2 border-dashed border-[#575767]">
    ...
    <template v-else>
      ...
      <div v-if="errors?.logo && errors?.logo.length > 0">
        <p v-for="(err, index) in errors?.logo" :key="index" class="mt-2 text-dangerColor">{{ err }}</p>
      </div>
      <div v-else class="text-sm text-muted-foreground mt-4">1:1 aspect ratio recommended</div>
    </template>
  </div>
</template>
...
import { updateCategoryLogo } from '@/webServices/categoryService'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const logoUrl = ref(props.category?.logo_url)
    const file = ref(null)
    ...
    const onSubmit = async () => {
      ...
      const res = await updateCategoryLogo(props.id, formData, progress => {
