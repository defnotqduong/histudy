<template>
  <div class="mt-4 py-4 px-6 bg-lighterColor rounded-md">
    ...
    <div v-if="!isEditting" class="mt-2 text-headingColor">{{ name }}</div>
...
import { updateCategory } from '@/webServices/categoryService'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const originalName = ref(props.category?.name)
    ...
    const onSubmit = async () => {
      ...
      if (!name.value.trim()) {
        ...
        errors.value.name.push('Please enter a category name')
        isSubmitting.value = false
        return
      }

      const res = await updateCategory(props.id, { name: name.value })
    ...
    return { name, isEditting, isSubmitting, errors, toggleEdit, onSubmit }
  }
})
</script>
